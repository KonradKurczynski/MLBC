...
``` > install.packages("MLBC") ```  into the R console. The core functions of the package are:
 ...
 ## `ols_bca`
This procedure first computes the standard OLS estimator in a regression of an outcome (`Y`) on a design matrix (`Xhat`), the first column of which contains AI/ML-generated binary labels, and then applies an additive correction based on an estimate (`fpr`) of the false-positive rate computed externally. Standard errors are adjusted to account for uncertainty in the false-positive rate estimate.

**Parameters**

- **`Y`**: Numeric vector of response variables.
- **`Xhat`**: Numeric matrix of covariates. The first column must be the AI/ML-generated variable to correct.
- **`fpr`**: Numeric. Estimated false-positive rate of the generated regressor.
- **`m`**: Integer. Size of the validation sample used to estimate `fpr`.
...
- **`coef`**: Numeric vector of bias-corrected estimates (intercept first, if requested).
...
- **`Y`**: Numeric vector of response variables.
- **`Xhat`**: Numeric matrix of covariates excluding the intercept. The first column must be the AI/ML-generated variable to correct.
...
- **`distribution`**: Character. One of `"normal"`, `"t"`, `"laplace"`, `"gamma"`, or `"beta"`. Specifies which conditional density to use for residuals in maximum likelihood estimation.
- **`nu`**: Numeric. Degrees of freedom (only used if `distribution = "t"`).
- **`gshape, gscale`**: Numeric. Shape & scale for gamma (only used if `distribution = "gamma"`).
